...
int main(void){
    ...
    for (int i=0;i<3000;i++){
        ...
        float randoma = static_cast<float>(std::rand()) / RAND_MAX;
        random.setColor(randomr,randomg,randomb,randoma);
        random.putVoxel(std::rand() % 51,std::rand() % 51,std::rand() % 51);
    }
    random.writeOFF("random.off");
    return 0;
}
